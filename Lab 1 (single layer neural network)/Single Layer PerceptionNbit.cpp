...
int main()
{
    float weight[100],tr=0.5,sum=0.0,r=0.0,f,eta=0.4;
    int i,j,k,l,p,n,t;
    ...
    for(i=0;i<n;i++)
    {
        weight[i]=((float) rand() / (RAND_MAX))+0.45;
        cout<<weight[i]<<endl;
    }
    g=pow(2,n);
    for(t=1;t<g;t++)
    ...
    for(l=0;l<5;l++)
    {
        ...
        for(i=g-2;i>=g/2;i--)
        {
            ...
            if(sum>=tr)
            {
                    //cout<<"Before"<<k<<weight[k]<<endl;
                    weight[k]=weight[k]-eta*input[i][k];
                    //cout<<"after"<<k<<weight[k]<<endl;
                    goto start;
            }
